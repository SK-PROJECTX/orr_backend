...
from ..serializers.ai_oversight import (
    AIConversationListSerializer, AIConversationDetailSerializer,
...
class AIConversationListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if needs_improvement is not None:
            queryset = queryset.filter(needs_improvement=needs_improvement.lower() == 'true')
...
class AIConversationDetailView(generics.RetrieveAPIView):
    ...
    queryset = AIConversation.objects.select_related('client__user', 'reviewed_by').all()
...
class AIConversationStatsView(APIView):
    ...
    def get(self, request):
        ...
        conversations_7d = AIConversation.objects.filter(created_at__gte=last_7_days).count()
        conversations_30d = AIConversation.objects.filter(created_at__gte=last_30_days).count()
        
        # Escalation metrics
        total_escalated = AIConversation.objects.filter(escalated_to_ticket=True).count()
        ...
        escalation_rate_overall = (total_escalated / total_conversations * 100) if total_conversations > 0 else 0
        escalation_rate_7d = (escalated_7d / conversations_7d * 100) if conversations_7d > 0 else 0
        
        # Improvement metrics
        needs_improvement = AIConversation.objects.filter(needs_improvement=True).count()
