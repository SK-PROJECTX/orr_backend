...
class MeetingStatsView(APIView):
    ...
    def get(self, request):
        ...
        meetings_by_type = dict(
            Meeting.objects.values('meeting_type').annotate(count=Count('id')).values_list('meeting_type', 'count')
        )
...
class MyMeetingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ).select_related('client__user').order_by('confirmed_datetime')
...
class UpcomingMeetingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Meeting.objects.filter(
            ...
            confirmed_datetime__lte=timezone.now() + timedelta(days=7),
            status='confirmed'
        ).select_related('client__user', 'host').order_by('confirmed_datetime')