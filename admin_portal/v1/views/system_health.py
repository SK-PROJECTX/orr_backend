...
class SystemPerformanceView(APIView):
    ...
    def get(self, request):
        ...
        recent_activities = {
            'audit_logs_last_hour': AuditLog.objects.filter(timestamp__gte=last_hour).count(),
            'notifications_last_hour': SystemNotification.objects.filter(created_at__gte=last_hour).count(),
            'database_queries': queries_count,
        }
        
        return Response({
            'performance_metrics': recent_activities,
