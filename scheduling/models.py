...
class Calendar(Audit):
    ...
    owner_user = models.OneToOneField(
        User, null=True, blank=True, on_delete=models.CASCADE, related_name="calendars"
    )
...
class MeetingRequest(Audit):
    requester = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="meeting_requests"
    )
    calendar = models.ForeignKey(
        "Calendar", on_delete=models.CASCADE, related_name="meeting_requests"
    )
    meeting_type = models.CharField(
        max_length=50, choices=MEETING_TYPE_CHOICES, default="first_meeting"
    )
