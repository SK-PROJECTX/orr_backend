...
class OnboardingQuestionnaireSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('jurisdiction') == 'other' and not data.get('jurisdiction_other', '').strip():
            raise serializers.ValidationError({"jurisdiction_other": "This field is required when 'Other' is selected."})
